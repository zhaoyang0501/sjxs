...
$(function(){   
	dg=$('#dg').treegrid({  
	method: "get",
    url:'${ctx}/account/accountType/json', 
    fit : true,
	fitColumns : true,
	border : false,
	idField : 'id',
...
function add() {
	...
	d=$('#dlg').dialog({    
	    ...
	    maximizable:true,
	    resizable:true,
	    href:'${ctx}/account/accountType/create',
	    modal: true,
	    buttons:[{
			text:'确认',
			handler:function(){
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/account/accountType/delete/"+row.id,
...
function upd(){
	...
	d=$("#dlg").dialog({   
	    ...
	    width: 450,    
	    height: 320,    
	    href:'${ctx}/account/accountType/update/'+row.id,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'确认',
